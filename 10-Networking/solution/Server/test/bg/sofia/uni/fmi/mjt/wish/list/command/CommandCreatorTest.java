...
public class CommandCreatorTest {
    ...
    public void testCommandCreationWithNoArguments() {
        ...
        assertEquals("unexpected command returned for command 'logout'", command, cmd.command());
    ...
    public void testCommandCreationWithOneArgument() {
        ...
        assertEquals("unexpected command returned for command 'test abcd'",
            command.split(" ")[0], cmd.command());
        assertNotNull("command arguments should not be null", cmd.arguments());
        assertEquals("unexpected command arguments count", 1, cmd.arguments().length);
        assertEquals("unexpected argument returned for command 'test abcd'",
            command.split(" ")[1], cmd.arguments()[0]);
    }
    ...
    public void testCommandCreationWithTwoArguments() {
        ...
        assertEquals("unexpected command returned for command 'test abcd 1234'",
            command.split(" ")[0], cmd.command());
        assertNotNull("command arguments should not be null", cmd.arguments());
        assertEquals("unexpected command arguments count", 2, cmd.arguments().length);
        assertEquals("multi-word argument is not respected", "abcd 1234",
